...
u8 hdmi_iic[][3] = {
  ...
  { 0x39, 0x02, 0x18 },
  { 0x39, 0x03, 0x00 },
  { 0x39, 0x15, /*0x00*/ 0x01 },	// 16, 20, 24 bit YCbCr 4:2:2 (separate syncs)
  { 0x39, 0x16, 0x3C + 0x01 },		// Output Format -> 0 = 4:4:4, Color Depth  -> 11 = 8 bit, Input Style -> 11 = style 3
  {	0x39, 0x17, 0x02 + 0x04},		// 4:2:2 to 4:4:4 Up Conversion Method -> 1 = use first order interpolation, Aspect Ratio -> 1 = 16:9 Aspect Ratio
  ...
  { 0x39, 0x40, 0x00 },
  { 0x39, 0x41, 0x10 },
  { 0x39, 0x48, 0x08 /*+ 0x40*/},				// Video Input Justification -> 01 = right justified
  { 0x39, 0x49, 0xA8 },
  { 0x39, 0x4C, 0x00 },
  { 0x39, 0x55, 0x00 },
  { 0x39, 0x56, 0x28 },
...
////	BytesWritten = EepromWriteByte(0x20, WriteBuffer, 0);
	...
	for (u8 Index = 0; Index < NUMBER_OF_HDMI_REGS; Index++) {
		EepromIicAddr = ADV7511_INIT_SEQUENCE[Index][0];
		BytesWritten = EepromWriteByte(ADV7511_INIT_SEQUENCE[Index][1], &ADV7511_INIT_SEQUENCE[Index][2],
				1);
...
//
	    /*BytesRead = */EepromReadByte(ADV7511_INIT_SEQUENCE[Index][1],
	    /*&ADV7511_INIT_SEQUENCE[Index][2]*/ &readReg, 1);
	    xil_printf("Register : %X = %X (%X)\n\r",	ADV7511_INIT_SEQUENCE[Index][1], readReg, ADV7511_INIT_SEQUENCE[Index][2]);
	    if (readReg != ADV7511_INIT_SEQUENCE[Index][2])
	    {
	    	xil_printf("\t\tERROR !!!\n\r");
	    }
	}

	EepromReadByte((AddressType)0x3D, &readReg, 1);
	xil_printf("VIC: %X = %X\n\r", 0x3D, readReg);
